...
plotDiagnostic <- function(out, lty = 1, lwd = 1, numSamples = 10){
  ...
  PSRFResult <- sapply(seq, FUN = function(x) coda::gelman.diag(getSample(out, parametersOnly = T, coda = T, end = x))$mpsrf)
  ...
  # plot DIC
  plot(ylim = c(min(DICResult), max(DICResult)), x = 0, xlim = c(1,lenW), lty =  1, lwd = 1, type = "l", main = "DIC", xlab = "Iterations", ylab = "")
  if(is.matrix(DICResult)){
    for(i in 1:nrow(DICResult)){
      lines(y = DICResult[i,], x = 1:ncol(DICResult), lty = 1, lwd = 1, col = rainbow(nrow(DICResult))[i])
    }
  } else {
    lines(y = DICResult, x = 1:length(DICResult), lty = 1, lwd = 1, col = "red")
  }
  
  # plot WAIC
  if(calcWAIC){
    plot(ylim = c(min(WAICResult), max(WAICResult)), x = 0, xlim = c(1,lenW),lty =  1, lwd = 1, type = "l", main = "WAIC", xlab = "Iterations", ylab = "")
    if(is.matrix(WAICResult)){
      for(i in 1:nrow(WAICResult)){
        lines(y = WAICResult[i,], x = 1:ncol(WAICResult), lty = 1, lwd = 1, col = rainbow(nrow(WAICResult))[i])
      }
    } else {
      lines(y = WAICResult, x = 1:length(WAICResult), lty = 1, lwd = 1, col = "blue")
    }
  }
  
  # plot mPSRF
  if(!typeof(seq) == "logical") plot(y = PSRFResult, x = seq, lty =  1, lwd = 1, type = "l", main = "mPSRF", xlab = "Iterations", ylab = "" )

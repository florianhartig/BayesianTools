...
#' 
#' @details This is the general prior generator (factory) in BayesianTools.
#' 
#' A prior encodes the information on the model parameters that is available a priori, i.e. before confronting the model with observed data. Thus, if one knows very little about a parameter, one will tend to set a wide prior, wheras in the case of detailed information, one will set a narrow prior. 
#' 
#' To specify a prior in BT, you have to provide a n-dimensional density functions, and optionally, but highly recommended, a sampler function that generates draws from the prior density. This sampler function is used to generate start values in the MCMCs. In no sampler is provided, the user will have to provide explicit starting values for many of the MCMC samplers. 
#' 
#' Note the the BT package provides several pre-existing prior function such as [createUniformPrior], [createTruncatedNormalPrior] or [createBetaPrior]. If your prior fits to those functions, their use is preferred. 
#' 
#' Moreover, priors can be created from an existing sample (e.g. generated by an MCMC or SMC sampler)via \code{\link{createPriorDensity}}. This facilitates the Bayesian cycle of a past posterior being a new prior. 
#' 

...
---
title: "Comparison of sampler convergence"
author: "TG14"
---
...
This document shows an example for how to run the different MCMC algorithms for the study in TG14 with the VSEM toy model that is provided in the statistics R package of PROFOUND (BayesianTools).  

* Note: if your runtime is limited you can break down the iterations into smaller chunks.
At the end of the document you will find an explanation of how to do this.

* **Note: if you need more background, see the [TG13 tutorial](https://www.dropbox.com/s/y3d5fqh61eqdzz6/TG13-ModelCalibrationTutorial.pdf?dl=0)**
...
Load the BayesianTools package
...
It aids reproducibility and us if you do a sessinInfo at the beginning of your script
...
# Add noise
obs <- apply(referenceData, 2, function(x) x + rnorm(length(x), sd = abs(x) * refPars$best[12]))
```
...
For each sampler three chains should be run. This not only gives a better data basis but is also necessary for the convergence diagnostic. 
...
The chains are stored in a 'coda::mcmc.list'. This allows to immediately apply the diagnostic functions in
the coda package. Note that in "DREAMzs" and "Twalk" the sampler output already consists of mcmc.lists with the different chains. Here all three mcmc.lists will be saved. However, the code will not differ from the code used here for the Metropolis sampler.

```{r, cache = TRUE, warning=F, message=F, results = 'hide'}
resMH <- createMcmcSamplerList(chainMH1, chainMH2, chainMH3)
```

Before we will continue, the results are stored on disc as a .RData file. These files will be your contribution to TG14. 
...
chainDREAM2 <- runMCMC(bayesianSetup = chainDREAM1, sampler ="DREAMzs", settings = settings)
chainDREAM3 <- runMCMC(bayesianSetup = chainDREAM1, sampler ="DREAMzs", settings = settings)

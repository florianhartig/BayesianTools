...
load("./out/reference_VSEMa_list.RData")
...
out.dir <- paste0("/home/fr/fr_fr/fr_ms1719/out/VSEMa", number, "/")
...
for(i in 1:nrow(settings)){
  cur.time <- system.time(smc.VSEMa <- smcSampler(bayesianSetup = bayesianSetup, initialParticles = settings$particles[i], ess.limit = (settings$particles[i] * settings$ess.limit[i]), proposalScale=settings$proposalScale[i], mutate = "DE", resamplingSteps = settings$resamplingSteps[i]))
  dist.d <- getSampleDistance(ref.sample, getSample(smc.VSEMa), type = "D")
